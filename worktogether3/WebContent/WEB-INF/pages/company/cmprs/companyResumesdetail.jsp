...
<div class="panel panel-default">
		...
		<c:forEach items="${companyResumes }" var="cmprs">
			<div class="media">
				...
				<div class="media-body"
					...
					<!-- 年龄和工作年限需要计算 -->
					<p class="static">
						${SEX[cmprs.resumeGender]}
						<fmt:formatDate value="${cmprs.resumeBirthday }"
							pattern="yyyy/MM/dd" />
						&nbsp;&nbsp;${cmprs.resumeJor }
					</p>
					<label class="control-label"><span
						...
						class="glyphicon glyphicon-earphone"></span>${cmprs.resumePhone }</label>
				</div>
				<!-- 简历基本信息结束 -->
			</div>
							...
							<div class="col-md-6 column">
								<label class="control-label">婚姻状况：</label> <label
									class="control-label"> <c:if
										test="${cmprs.resumeMarriage==0 }">已婚
									</c:if> <c:if test="${cmprs.resumeMarriage==1 }">未婚
									</c:if> </label>
							</div>
							<div class="col-md-6 column">
							...
							<div class="col-md-6 column">
								<label class="control-label">毕业时间：</label><label
									class="control-label"><fmt:formatDate
										value="${cmprs.resumeGraduationTime }" pattern="yyyy/MM/dd" />
								</label>
							</div>
							<div class="col-md-6 column">
								<label class="control-label">学历：</label><label
									class="control-label">${cmprs.resumeEducation } </label>
							</div>
							<div class="col-md-6 column">
								<label class="control-label">专业：</label><label
									class="control-label">${cmprs.resumeMajor } </label>
							</div>
							<div class="col-md-6 column">
								<label class="control-label">外语水平：</label><label
									class="control-label"> <c:if
										test="${cmprs.resumeFLAbility==1 }">入门
									</c:if> <c:if test="${cmprs.resumeFLAbility==2 }">一般
									</c:if> <c:if test="${cmprs.resumeFLAbility==3 }">熟练
									</c:if> <c:if test="${cmprs.resumeFLAbility==4 }">精通
									</c:if> </label>
							</div>
							<div class="col-md-6 column">
								<label class="control-label">外语类型：</label><label
									class="control-label"> <c:if
										test="${cmprs.resumeFLType==1 }">英语
									</c:if> <c:if test="${cmprs.resumeFLType==2 }">日语
									</c:if> <c:if test="${cmprs.resumeFLType==3 }">德语
									</c:if> <c:if test="${cmprs.resumeFLType==4 }">法语
									</c:if> <c:if test="${cmprs.resumeFLType==5 }">俄语
									</c:if> <c:if test="${cmprs.resumeFLType==6 }">西班牙语
									</c:if> <c:if test="${cmprs.resumeFLType==7 }">丹麦语
									</c:if> </label>
							</div>
							<div class="col-md-6 column">
								<label class="control-label">英语等级：</label><label
									class="control-label"> <c:if
										test="${cmprs.resumeEnglishGrade==1 }">国家英语考试3级
									</c:if> <c:if test="${cmprs.resumeEnglishGrade==2 }">国家英语考试4级
									</c:if> <c:if test="${cmprs.resumeEnglishGrade==3 }">国家英语考试6级
									</c:if> <c:if test="${cmprs.resumeEnglishGrade==4 }">专业4级
									</c:if> <c:if test="${cmprs.resumeEnglishGrade==5 }">专业8级
									</c:if> <c:if test="${cmprs.resumeEnglishGrade==6 }">未参加
									</c:if> <c:if test="${cmprs.resumeEnglishGrade==7 }">未通过
									</c:if> </label>
							</div>
							<div class="col-md-6 column">
								<label class="control-label">计算机水平：</label><label
									class="control-label"> <c:if
										test="${cmprs.resumeComputerAbility==1 }">入门
									</c:if> <c:if test="${cmprs.resumeComputerAbility==2 }">一般
									</c:if> <c:if test="${cmprs.resumeComputerAbility==3 }">熟练
									</c:if> <c:if test="${cmprs.resumeComputerAbility==4 }">精通
									</c:if> </label>
							</div>
							<div class="col-md-6 column">
		...
		<div class="panel-body col-md-12 test personal_resume_defualt">
			<c:if test="${works!=null }">
				<c:forEach items="${works }" var="works">
					<div class="col-md-6 column">
						<label class="control-label">企业名称：</label> <label
							class="control-label">${works.workFirmName }</label>
					</div>
					<div class="col-md-6 column">
						<label class="control-label">工作岗位：</label> <label
							class="control-label">${works.workUnit }</label>
					</div>
					<div class="col-md-6 column">
						<label class="control-label">工作描述：</label> <label
							class="control-label">${works.workDescription }</label>
					</div>
					<div class="col-md-6 column">
						<label class="control-label">工作时间：</label> <label
							class="control-label"><fmt:formatDate
								value="${works.workBeginTime }" pattern="yyyy-MM-dd" />到<fmt:formatDate
								value="${works.workEndTime }" pattern="yyyy-MM-dd" /> </label>
					</div>
				</c:forEach>
			</c:if>
		</div>
	...
	<div class="panel-body personal_resume_margin ">
		<div class="panel-body col-md-12 test personal_resume_defualt">
			<c:if test="${projs!=null }">
				<c:forEach items="${projs }" var="projs">
					<div class="col-md-6 column">
						<label class="control-label">项目名称：</label> <label
							class="control-label">${projs.projectName }</label>
					</div>
					<div class="col-md-6 column">
						<label class="control-label">项目岗位：</label> <label
							class="control-label">${projs.projectJob }</label>
					</div>
					<div class="col-md-6 column">
						<label class="control-label">项目描述：</label> <label
							class="control-label">${projs.projectDescription }</label>
					</div>
					<div class="col-md-6 column">
						<label class="control-label">项目时间：</label> <label
							class="control-label"> <fmt:formatDate
								value="${projs.projectBeginTime }" pattern="yyyy-MM-dd" />到<fmt:formatDate
								value="${projs.projectEndTime }" pattern="yyyy-MM-dd" /> </label>
					</div>
				</c:forEach>
			</c:if>
		</div>
	...
	<div class="panel-body personal_resume_margin ">
		<div class="panel-body col-md-12 test personal_resume_defualt">
			<c:if test="${edus!=null }">
				<c:forEach items="${edus }" var="edus">
					<div class="col-md-6 column">
						<label class="control-label">学校名称：</label> <label
							class="control-label">${edus.educationSchool }</label>
					</div>
					<div class="col-md-6 column">
						<label class="control-label">教育描述：</label> <label
							class="control-label">${edus.educationContent}</label>
					</div>
					<div class="col-md-6 column">
						<label class="control-label">教育时间：</label> <label
							class="control-label"><fmt:formatDate
								value="${edus.educationBeginTime}" pattern="yyyy-MM-dd" />到<fmt:formatDate
								value="${edus.educationEndTime}" pattern="yyyy-MM-dd" />
						</label>
					</div>
				</c:forEach>
			</c:if>
	...
	<%@include file="/WEB-INF/pages/company/offer/add_offer.jsp" %>
</div>