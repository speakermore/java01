...
<table class="table table-hover table-striped">
	...
	<c:if test="${mylist!=null}">
		<c:forEach items="${mylist}" var="cf">
			<tr>
				<td><a title="查看"
					href="crowdfund/crowdfundOnlyInfo?id=${cf.id}">${cf.crowdfundProjectName}</a></td>
				<td><c:if test="${cf.publisherId<1234567891}">
						企业
					</c:if> <c:if test="${cf.publisherId>1234567890}">
						个人
					</c:if></td>
				<td><fmt:formatNumber value="${cf.crowdfundProjectMoney}"
						pattern="##,###,###.00元"></fmt:formatNumber></td>
				<c:if test="${cf.crowdfundProjectEndDate.time>=now.time}">
					<td><fmt:formatDate value="${cf.crowdfundProjectStartDate}"
							pattern="yyyy年MM月dd日 E" /></td>
				</c:if>
				<c:if test="${cf.crowdfundProjectEndDate.time<=now.time}">
					<td>已失效</td>
				</c:if>
				<c:if test="${cf.crowdfundProjectCheckState==1}">
					<td>审核中</td>
					<td><a title="管理" href="crowdfund/updateCrowdfund?id=${cf.id}"><span
							class="glyphicon glyphicon-pencil"></span></a> <a title="删除"
						onClick="return confirm('您真的确定要撤回吗?')"
						href="crowdfund/deleteCrowdfund?id=${cf.id}"><span
							class="glyphicon glyphicon-trash"></span></a></td>
				</c:if>
				<c:if test="${cf.crowdfundProjectCheckState==2}">
					<td>审核通过</td>
					<td><a class="nopointer" style="color: #cccccc;" title="禁止修改"
						href="javascript:void(0)"><span
							class="glyphicon glyphicon-pencil"></span></a> <a title="删除"
						onClick="return confirm('您确定要删除该众筹信息？')"
						href="crowdfund/deleteCrowdfund?id=${cf.id}"><span
							class="glyphicon glyphicon-trash"></span></a></td>
				</c:if>
