...
public class CompanyRecruitController {
	...
	//查询所有招聘信息
	 ...
	 public ModelAndView findAll(Integer page,HttpSession session){
		 Integer companyId=((Company)session.getAttribute("user")).getId();
		 Integer companyRecruitId=companyRecruitService.findCompanyRecruitId();
		 Integer resumeCount=companyRecruitService.findCount(companyRecruitId);
		 ...
		 mv.addObject("companyRecruitId",companyRecruitId);
	 ...
	 @RequestMapping("/companyRecruit/recover")
	 public ModelAndView recover(Integer id){
		 int result=companyRecruitService.recover(id);
		 ModelAndView mv=new ModelAndView();
		 if(result>0){
			 mv.addObject("operatorInfo","修改成功");
				mv.addObject("toPage","company/cmprs/companyRecruit/findAll");
